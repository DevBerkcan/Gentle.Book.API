...
public class SuperAdminController : ControllerBase
{
    ...
    /// <summary>
    /// Maintenance cleanup: remove one leaked employee from every tenant except a protected tenant.
    /// Bookings are preserved and detached from the employee.
    /// </summary>
    [HttpDelete("maintenance/employees/{employeeId:guid}")]
    public async Task<IActionResult> DeleteLeakedEmployee(
        Guid employeeId,
        [FromQuery] string preserveSlug = "hautliebe")
    {
        if (ForbidIfNotSuperAdmin() is { } err) return err;

        var protectedSlug = preserveSlug.Trim().ToLowerInvariant();
        var protectedTenantId = await _db.Tenants
            .AsNoTracking()
            .Where(t => t.Slug.ToLower() == protectedSlug)
            .Select(t => (Guid?)t.Id)
            .FirstOrDefaultAsync();
        ...
        var employees = await _db.Employees
            .IgnoreQueryFilters()
            .Include(e => e.Tenant)
            .Where(e => e.Id == employeeId && (!protectedTenantId.HasValue || e.TenantId != protectedTenantId.Value))
            .Select(e => new
            {
                e.Id,
                e.Name,
        ...
        if (!employees.Any())
        {
            return NotFound(new
            {
                message = "Kein passender Mitarbeiter ausserhalb des geschuetzten Tenants gefunden.",
                employeeId,
                preserveSlug = protectedSlug
            });
        }
        ...
        try
        {
            var detachedBookings = await _db.Bookings
                .IgnoreQueryFilters()
                .Where(b => b.EmployeeId.HasValue && employeeIds.Contains(b.EmployeeId.Value))
                .ExecuteUpdateAsync(setters => setters
                    .SetProperty(b => b.EmployeeId, (Guid?)null)
                    .SetProperty(b => b.UpdatedAt, DateTime.UtcNow));

            var deletedBlockedSlots = await _db.BlockedTimeSlots
                .IgnoreQueryFilters()
                .Where(b => b.EmployeeId.HasValue && employeeIds.Contains(b.EmployeeId.Value))
                .ExecuteDeleteAsync();

            var deletedVacations = await _db.EmployeeVacations
                .IgnoreQueryFilters()
                .Where(v => employeeIds.Contains(v.EmployeeId))
                .ExecuteDeleteAsync();

            var deletedSchedules = await _db.EmployeeSchedules
                .IgnoreQueryFilters()
                .Where(s => employeeIds.Contains(s.EmployeeId))
                .ExecuteDeleteAsync();

            var deletedAssignments = await _db.ServiceEmployees
                .IgnoreQueryFilters()
                .Where(se => employeeIds.Contains(se.EmployeeId))
                .ExecuteDeleteAsync();

            var deletedEmployees = await _db.Employees
                .IgnoreQueryFilters()
                .Where(e => employeeIds.Contains(e.Id) && (!protectedTenantId.HasValue || e.TenantId != protectedTenantId.Value))
                .ExecuteDeleteAsync();

            await tx.CommitAsync();

            _logger.LogWarning(
                "Maintenance deleted leaked employee {EmployeeId}; employees={DeletedEmployees}, assignments={DeletedAssignments}, schedules={DeletedSchedules}, vacations={DeletedVacations}, blockedSlots={DeletedBlockedSlots}, detachedBookings={DetachedBookings}, preserveSlug={PreserveSlug}",
                employeeId, deletedEmployees, deletedAssignments, deletedSchedules, deletedVacations, deletedBlockedSlots, detachedBookings, protectedSlug);
